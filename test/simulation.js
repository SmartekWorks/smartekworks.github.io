...
$(function () {
    ...
    var styleHandler = new SimulationStyle(); // シミュレーション用スタイル制御オブジェクト生成

    $('#3').text((new Date()).getTime());
    // SiteCatalyst通信サンプル(動的に設定する必要のあるプロパティをJSONオブジェクトで渡す)
    var propertyText = '{"prop":{"prop31":"男", "prop32":"年齢"}}';
    var property = JSON.parse(propertyText);
    // app.callSiteCatalystOnLoad("/api/site-catalyst/get-json", "simulation", property);

    
    // 画面のhidden値から初期化に必要な情報を取得する
    var initializeProperty = JSON.parse($('#initializeProperty').val());
    
    // estimate経由でない場合、デフォルトの商品を設定
    ...
    // Todo: simulation.viewmodelを作成しHTMLにバインディングする
    // ViewModelのインスタンス化
    var viewModel = new SimulationViewModel(app, styleHandler, resultData.content, resultConst.content);
    ko.applyBindings(viewModel);    // Knockout起動(HTMLのルートノードにマッピング)
    $('#7').text((new Date()).getTime());

    // 初期化処理(プルダウン生成、契約年齢計算、口座メンテナンス情報計算など)
    viewModel.init(resultData.content);
    $('#8').text((new Date()).getTime());
});
