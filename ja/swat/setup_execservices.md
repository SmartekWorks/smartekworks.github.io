...
* Webアプリケーションでベーシック認証を使用する場合は、`http://<username>:<password>@yourdomain`のようなフォーマットでURLを使うことができます。しかしながら、IEはデフォルトでこのタイプのURLをサポートしません。ターゲット端末にレジストリを入力しなければなりません。 レジストリファイルは[ここ](http://www.smartekworks.com/tools/ie-enable-basic-auth.zip)からダウンロードできます。
