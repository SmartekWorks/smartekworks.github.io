...
マッチングルールとクエリールールは、Web画面上のデータを検証、取得するために使用されています。単純なシナリオを構築し、それをブラウザで自動的に実行したい場合は、いずれかのルールを使用する必要はありません。しかしながら、テスト効率を向上させ、複雑なシナリオ構築を支援するためには、これらのルールが必要です。
...
[**画面検証**](ref_sys_operation.md#オペレーション_-_画面検証)システムオペレーションが自動検証のチェックポイントとして、シナリオへ追加することが可能です。このシステムオペレーションのパラメータとしてマッチングルールをセットする必要があります。実行するときに、SWATはウィンドウ内の画面でマッチングルールが通るかどうかどうかチェックします。もし条件が満たされない場合、エラーを報告します。 適切な画面検証でシナリオを設計する場合には、手動ですべてのスクリーンショットをチェックせずにテストが成功かどうかがわかります。

自動検証を構築するには工夫が必要かもしれませんが、それが大幅にテスト効率を向上させることは可能です。特に回帰テストの場合、自動検証がほとんど必要とされます。

#### ブラウザウィンドウを特定

[**ウィンドウ制御**](ref_sys_operation.md#オペレーション_-_ウィンドウ制御)システムオペレーションを利用すると、ターゲットウィンドウを見つけて、それに対して操作することができます。このシステムオペレーションのパラメータとしてマッチングルールをセットする必要があり、それを実行するとき、SWATはマッチングルールが通った最初ウィンドウに切り替えたり、それを閉じたりすることが可能です。
...
画面識別情報を定義したなら、その画面上でオペレーションを実行する前に、マッチングルールによる画面検証を行います。それは、ターゲット画面に似ていて間違った画面で操作を実行しないようにするのに役立ちます。 
...
```
 * `"equal_text"`: 評価対象のテキストは、キーの文字列値に等しくなければならない。
 * `"not_equal_text"`: 評価対象のテキストは、キーの文字列値に等しくない。
 * `"have_text"`: 評価対象のテキストは、キーの文字列値に含まれる。
 * `"not_have_text"`: 評価対象のテキストは、キーの文字列値に含まれない。
 * `"have_css"`: 評価対象のエリアは、定義されたノードに含まれる。`"target"`は*CSSセレクター*の場合にのみ有効です。
 * `"not_have_css"`: 評価対象のエリアは、定義されたノードに含まれない。`"target"`は*CSSセレクター*の場合にのみ有効です。
...
クエリルールは、画面から値を取得するために使用することができるドメイン固有言語の一つで書かれています。SWATの以下機能でクエリ-ルールを使うことができます。

#### Web画面からの値を使い回す

複雑なシナリオでは、前のオペレーションの画面からの値を後続のオペレーションで使う必要があるかもしれません。例えば、*注文完了*画面で表示される*ご注文番号*を注文ステータス検索するために使うこととか。

このような場合、[**画面変数設定**](ref_sys_operation.md#オペレーション_-_画面変数設定)システムオペレーションでWeb画面から値を取得し、後のオペレーションのパラメータで使用できる変数に保存すべきです。 このオペレーションのパラメータとしてクエリ-ルールをセットすることが必要で、それを実行している時、SWATはこのルールに従って値を取得します。
...
```
* `"css_key"`: **（`"source"`が*CSSセレクター*の場合、必須）** ノードの属性名。また、ノードのテキストを取得するために`text`を使用することができます。
```json
{"source":"#selectGender", "css_key":"value"}
```
