...
SWATのナレッジルールは、Web画面の実装と画面上の各種オペレーションをSWATに理解させるための規則(ルール)です。さまざまなWebアプリケーションのWeb画面をSWATが理解するためには、柔軟なナレッジルールが必要となります。

なぜナレッジルールが必要か？ 
---

SWATはWeb画面を解析しそのWeb画面を理解しますが、理解するためのルールがあります。ナレッジルールは一般的なWeb画面の作りをデフォルトのルールとします。但し、以下の場合はデフォルトのナレッジルールを変更することができます。

* 非インタラクティブなHTMLタグで構成されたオペレーションを追加する場合。JavascriptやJSのライブラリを使用して`div`などの非インタラクティブなHTMLタグを利用する場合です。[スタートガイド](guide_tuning.md#画面ナレッジチューニング)でサンプルが参照できます。
* デフォルトのオペレーションサイズを変更する場合。デフォルトルールでは一般的な使いやすさに応じて適切なサイズでのオペレーションを抽出します。しかし、時には複数のオペレーションをマージしたり、いくつか小さな操作に分割するなどの場合はデフォルトルールの変更が必要です。
* 実行中にオペレーションの場所を特定する方法を変更する場合。自動化のケースが動的画面上で実行される場合、信頼性の高い自動化のためのロケーティング方針が必要です。殆どのWebアプリケーションは`id`や`name`などで統一されたロケーティング方針が持ちません。その場合、ナレッジルールで柔軟にロケーティング方法を定義することができます。
* オペレーションとノードのラベルを取得する方法を変更する場合。SWATはオペレーションとノードを抽出する際にHTMLからラベルを取得しようとします。Webアプリケーションの特性によってより適切なラベル取得方法を定義することができます。
...
ナレッジルールは、Webアプリケーション配下の全てのWeb画面で設定されます。ナレッジルールは、SWATサイトの下で管理されます。Webアプリケーションの実装にいろいろなタイプがあるため、SWATサイトの下にいくつかのルールを設定することができます。保守性を考えるとあまりお勧めしませんが、特定のWeb画面に対するルールを追加することもできます。

SWATの画面ナレッジとナレッジルールは関連性を持っており、そのルールに従い画面ナレッジが構成されます。ナレッジルールの変更を行う場合は新たなルールを追加し、画面ナレッジをアップグレードする必要うがあります。
...
ラベルポリシーはJSONマップで定義します。 キーはコード（ノード定義で参照するコード）で値はラベルポリシーの定義です。下記サンプルを参照ください。 
...
デフォルトのナレッジルールは、標準のHTML仕様に作られた画面をうまく解析できますが、UIライブラリやJSで作られた画面の操作要素を利用するWeb画面の場合、このルールを拡張して解析することができます。 
