...
シナリオかフローの作成で画面オペレーションを使うのはシンプルでわかりやすいです。画面オペレーションをドラッグ＆ドロップし、オペレーションの実行に必要なパラメータを入力するだけです。一つのパラメータは**タイトル**、**使用オプション**、 **パラメータデータ**、 **クエリモードスイッチ**と**アラート処理**5つの部分で構成されます。
...
* `Text`: テキストエリアやテキストボックスなどのようなテキストタイプノードで入力するタイプ。
* `Select`: セレクトタイプのノードにオプションを選択するタイプ。ラジオグループや、リンクかボタンのグループなどの相互に排他的な入力を持つコレクションもこのタイプに属します。
* `Multi-select`: マルチセレクトタイプのノードに複数オプションを選択するタイプ。チェックボックスグループなどの非排他的な入力を持つコレクションもこのタイプに属します。
* `Execute`: 単一のリンクとしてパラメータを指定する必要のないタイプ。
...
画面オペレーションは、検証中や実行中にさまざまなエラーが発生する可能性があります。もっとも共通的なエラーを以下に記します。
...
すべての画面オペレーションは、実行後のアクティブウィンドウからスクリーンショットとHTMLを基本的なエビデンスとして取ります。もしオペレーションが、入力やフォームのサブミットなどのいくつかのステップが含まれている場合、最後のステップの前にもう一つのスクリーンショットを取得します。また、オペレーションの中に任意のアラート処理があった場合アラートのエビデンスも利用可能になります。
...
もし多ウインドウで、他のウィンドウ画面でエビデンスを取得したい場合、**ウィンドウ制御**システムオペレーションで対象ウィンドウ画面に切り替える必要があります。

変数の使用
---

データとシナリオを分離するために、パラメータデータフィールドに`@{変数名}`の形式で変数を設定することができます。**画面変数設定**オペレーションと**外部API呼び出し**オペレーションで設定した変数の除く、シナリオ/フローのすべての変数がそのシナリオ/フローのパラメータになります。ケースのデータをオペレーション実行中で利用できます。
...
1. シナリオ/フロービルダーで、パラメータの**使用オプション**のチェックを外す。
2. ケース新規・編集ダイアログ該当パラメータにあるすべての変数の**使用オプション**のチェックを外す。（Excelの出力・インポート機能でも利用可能です。）
3. 親ノードのパラメータを無視する。例えば、各行に入力を持つテーブル操作などの複雑なネストされたノードの動作の場合、行を選択する親ノードを無視すると、子ノード（行の入力）も実行時に無視されます。
4. オペレーションを無視する。この場合、すべてのパラメータが無視されます。

オペレーションの無視
---

シナリオ/フロー·ビルダーでオペレーションを無視する意味がないが、ケースのデータ設定によって、オペレーションを無視する必要があるかもしれません。 
...
オペレーションは無視されている場合、テスト結果のエビデンスに表示されません。
